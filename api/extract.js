/* ==========================================================================
   ...
   무엇을 보내는가 — 정확히
     행 데이터는 보내지 않습니다. 보내는 것은 스키마 카탈로그와 질문뿐입니다.
     다만 카탈로그에는 컬럼마다 min · max 가 들어 있고, 그 둘은 실제 측정값
     입니다. 단위 해석("1000 이상"이 mg/L 인지 g/L 인지)에 필요해서 넣었습니다.
     컬럼 45개 × 2 = 실측값 90개가 모델에 갑니다. 나머지 셀 값과 배치별
     측정치는 가지 않습니다.

     ★ "모델은 값을 전혀 볼 수 없다" 고 쓰지 않습니다. 그건 사실이 아니고,
       선언과 구현이 어긋나면 감사에서 걸리는 것은 문서가 아니라 시스템입니다.

   그래도 수치를 지어낼 수 없는 이유는 따로 있습니다
     모델은 슬롯만 돌려줍니다. 조회 · 필터 · 집계는 브라우저 코드가 하고,
     응답의 모든 수치는 그 결과 객체에서만 나옵니다. 모델이 min/max 를
     그대로 답에 실으려 해도, 서술 검증기가 그 수치가 이 조회 결과의
     근거인지 다시 대조합니다.
