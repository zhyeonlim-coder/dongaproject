...
window.AskRegression = (function () {
  ...
  const BATTERY = [
    ...
    { q: "뭘 물어볼 수 있어?",             kind: "meta" },
    { q: "미입력이 가장 많은 항목은?",        intent: "missing", kind: "missing" },
    { q: "Titer 3 이상인 배치",            applied: /≥ 3/ },
    ...
    { q: "그럼 정제는?",                   kind: "group",  scope: "DA-1234", applied: /직전 질의/ },
    { q: "그거 언제 배양한 거야?",           kind: "date",   scope: "DA-1234", applied: /직전 질의/ }
  ];
  ...
  const CONTEXT = [
    ...
    { q: "그거 언제 배양한 거야?",     scope: "DA-1234", rows: 18, kind: "date",  applied: /직전 질의/ },
    /* 이어받기 표시가 없으면 물려받지 않아야 합니다 */
    { q: "Titer 평균은?",           scope: "전체",     rows: 28 }
  ];

  /* ── B. 조건이 다르면 응답도 달라야 한다 ─────────────────────────────── */
  const DISTINCT = [
    ["Titer 3 이상인 배치", "Titer 상위 5개", "Titer 기록된 배치"],
    ["2024년 12월 Titer 평균", "2024년 11월 Titer 평균", "Titer 평균"]
  ];
  ...
  function run(table) {
    ...
    /* ── B. 조건이 다르면 응답도 달라야 한다 ──────────────────────────── */
    const dis = [];
    DISTINCT.forEach(function (set) {
      const heads = set.map(q => E.answer(q, { table: t }).headline);
      const uniq = new Set(heads).size;
      dis.push({ set: set, uniq: uniq, pass: uniq === set.length, heads: heads });
    });
    out.distinct = dis;
    out.checks.push({
      id: "B. 조건 구별", pass: dis.every(d => d.pass),
      detail: dis.map(d => d.set[0].slice(0, 14) + "… " + d.uniq + "/" + d.set.length + " 서로 다름").join(" · ")
    });
  ...
  function text(res) {
    ...
    L.push("");
    [["E. Phase 1 조건 파서", r.phase1], ["F. Phase 2 어휘 · 팀", r.phase2], ["G. 맥락 승계", r.context]]
  ...
  return { run: run, text: text, BATTERY: BATTERY, ANCHORS: ANCHORS, DISTINCT: DISTINCT };
})();
