...
(function () {
  ...
  const user = window.Shell.mount({ page: "meeting" });
  if (!user) return;
  ...
  function paintSubnav() {
    window.Shell.subnav([
      { label: "회의 보기", items: [{ key: "overview", ko: "개요", active: !view.detail }]
          .concat(window.DATA_TEAMS.map(t => ({
            key: t.id, ko: t.ko + " 상세", active: view.detail === t.id, color: t.color }))) },
      { label: "바로가기", items: [
        { ko: "대시보드", href: "dashboard.html" },
        { ko: "데이터 조회", href: "data.html" }
      ]}
    ], k => { view.detail = (k === "overview") ? null : k; render(); });
  }
  ...
  function render() {
    ...
    if (!sel.scopeId) {
      $("#body").innerHTML = '<div class="empty"><div class="empty-title">과제 또는 기반 Study를 선택하세요</div>' +
        '<div class="empty-body">선택하면 해당 범위 데이터가 일괄 로딩됩니다.</div></div>';
      return;
    }
    ...
    Promise.all([
      ...
      $("#page-title").textContent = "회의 모드 · " + desc.scope + (desc.study ? " / " + desc.study : "");
