...
(function () {
  ...
  /* 규격 메뉴를 보여도 되는가.
     ...
     devMode() 만으로는 부족합니다. dev 플래그는 localStorage 에 남아
     한 번 켜지면 계속 켜져 있어서, ?dev=1 을 썼던 브라우저에서는 연구원
     계정으로 로그인해도 관리자 메뉴가 그대로 보입니다.

     그래서 권한도 함께 봅니다 — 규격을 바꿀 수 있는 사람에게만 보입니다.
     연구원은 dev 플래그가 켜져 있든 말든 이 메뉴를 보지 않습니다. */
  function canSeeSpec() {
    if (!devMode()) return false;
    if (!window.Auth || !window.Auth.can) return true;   /* Auth 없는 환경 */
    return window.Auth.can("spec:write");
  }
  ...
  function paintSubnav() {
    window.Shell.subnav([
      { label: "Intelligence Hub", items: [
        ...
        /* 규격은 Pass/Fail 판정의 기준이라 연구원이 드나들 화면이 아닙니다.
           다만 기능을 없애면 규격이 0건인 채로 굳어 "실패한 배치 있어?" 류
           질문이 영영 판정 불가가 됩니다 — 메뉴에서만 빼고 등록 경로는
           그대로 둡니다. (관리자 + 규격 쓰기 권한일 때만) */
        .concat(canSeeSpec()
  ...
  function paint() {
    ...
    $("#hub-tabs").innerHTML = [["doe", "DoE 설계 & 분석"], ["ai", "AI 검색"],
      ...
      .concat(canSeeSpec() ? [["spec", "규격(Spec) 관리"]] : [])
    ...
    if (tab === "spec") {
      ...
      if (!canSeeSpec()) {
        const noPerm = window.Auth && window.Auth.can && !window.Auth.can("spec:write");
        host.innerHTML =
          '<section class="card"><div class="card-body">' +
          '<h2 class="card-title">규격(Spec) 관리는 관리자 화면으로 옮겼습니다</h2>' +
          '<p class="ask-note">규격은 Pass/Fail 판정의 기준이라 등록·수정은 ' +
          '규제업무 담당자가 관리자 화면에서 합니다. 연구원 화면에서는 판정 ' +
          '<b>결과</b>만 봅니다 — AI 검색에서 "실패한 배치 있어?" 처럼 물으시면 됩니다.<br>' +
          (noPerm
            ? '지금 계정에는 규격 쓰기 권한이 없어 관리자 화면으로도 열리지 않습니다. ' +
              '판정 기준을 바꿔야 하면 규제업무 담당자에게 요청해 주세요.'
            : '기능이 없어진 것은 아닙니다. 주소 끝에 <code>?dev=1</code> 을 붙여 여시면 됩니다.') +
          "</p>" +
          '<div style="margin-top:var(--s-3);display:flex;gap:8px;flex-wrap:wrap">' +
          (noPerm ? "" : '<button class="btn btn-accent" id="spec-open-admin">관리자 화면으로 열기</button>') +
